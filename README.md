...
`release/FormatDrop-1.0.1.exe`를 더블클릭하면 내장 로컬 서버와 기본 브라우저가 자동으로 실행됩니다. Python이나 별도의 설치가 필요하지 않습니다. 배포용 압축 파일과 SHA256 체크섬도 `release` 폴더에서 확인할 수 있습니다.
...
- 첫 파일을 추가한 뒤에도 화면 어디에나 파일을 드래그 앤 드롭해 계속 추가
